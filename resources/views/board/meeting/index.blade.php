    ...
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-center align-items-center">
            ...
            @elseif(config('app.company') == 'msez')
                <h1 class="display-2 mb-0 text-center"> <span><img src="/img/msez.png" width="50" height="50" alt="OMPL" class="mr-2"></span> Managlore SEZ Limited </h1>
            @endif
        </div>

        <div class="ma-4 text-center">
            <h5 class="text-uppercase"> 
            {{ $meeting->name }}{{ ["TH","ST","ND","RD","TH","TH","TH","TH","TH","TH"][$meeting->name%10] }} Meeting of the Board of Directors of the Company, to be held on
        ...
        <div class="my-4" >
            <div class="row my-4">
                <div class="col-12 col-sm-12 col-md-12 col-lg-7 col-xl-7">
                    <ul class="nav nav-tabs">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">Committee</a>
                            <div class="dropdown-menu" style="z-index:1">
                                @foreach(Auth::user()->committees as $option )
                                    <a class="dropdown-item @if($option->agendas->where('status', 'takenup')->count() == 0) disabled  @endif" href="/committee/meeting/user/{{ $option->id }}"> {{ $option->name }}</a>
                                @endforeach
                            </div>
                        </li>
                    </ul>
                ...
                <div class="col-12 col-sm-6 col-md-6 col-lg-3 col-xl-3">
                    <form action="/board/meeting/filter" method="post">
                    @csrf
            ...
            <table class="table table-hover table-bordered">
                <thead class="bg-{{ config('app.company') }} text-white">
                    <tr>
                        ...
                        <th class="text-center">RECOMMENDED THROUGH</th>
                ...
                <tbody>
                    ...
                    @foreach ($leave as $item)
                        <tr>
                            ...
                            <td> {{ $item->type }}</td>
                            <td> {{ $item->committee->name }} <br/> {{ \App\CommitteeAgenda::where('board_uid', $item->uid)->first() > 0 ? \App\CommitteeAgenda::where('board_uid', $item->uid)->first()->uid : '' }} </td>
                            <td> {{ $item->category }}</td>
                            <td> {{ $item->added_at }} </td>
                        </tr>
                    @endforeach
                        ...
                        <tr>
                            <td>
                                @if(Auth::user()->hasRole('admin'))
                                    <a target="_blank" href="/board/agenda/{{ $item->id }}"> {{ $item->uid }} </a>
                                @else 
                                    {{ $item->uid }}
                                @endif
                            </td>
                            ...
                            <td> Noting </td>
                            <td> {{ $item->committee->name }} <br/> {{ \App\CommitteeAgenda::where('board_uid', $item->uid)->first() ? \App\CommitteeAgenda::where('board_uid', $item->uid)->first()->uid : '' }} </td>
                    ...
                    {{-- <tr style="background: #d4615e75"><td colspan="6"> <h5> NOTINGS ( {{ $noting->count() }} {{ str_plural('ITEM', $noting->count() ) }} ) </h5> </td></tr> --}}
